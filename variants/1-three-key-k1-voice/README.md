# V1 Fn Bridge Edition：三键硬件，K1 语音输入

V1 使用 XIAO ESP32S3、INMP441 和三键板，但当前只启用 K1。这个兼容版本通过 F13 和 Fn Bridge 实现语音输入，需要安装 Mac 端 App 并授予辅助功能权限。

新安装建议优先使用不需要 Mac App 的 [V1 Direct HID Edition](../1-three-key-k1-voice-direct-hid/README.md)。如果 Direct 版在目标 macOS 或输入法版本上不兼容，再使用本版本。
