...
if CommandLine.arguments.contains("--audio") {
    ...
    guard let tap = CGEvent.tapCreate(tap: .cghidEventTap, place: .headInsertEventTap, options: .listenOnly, eventsOfInterest: mask, callback: { _, type, event, _ in
        ...
        // F13/Fn (K1), Return (K2), and Command+A/Backspace (K3).
        if [105, 63, 36, 55, 0, 51].contains(key) {
            let repeatFlag = event.getIntegerValueField(.keyboardEventAutorepeat)
            print("KEY type=\(type.rawValue) key=\(key) repeat=\(repeatFlag) flags=\(event.flags.rawValue) sourcePID=\(event.getIntegerValueField(.eventSourceUnixProcessID))")
        }
