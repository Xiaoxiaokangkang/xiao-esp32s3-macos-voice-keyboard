...
constexpr int kTalkButton = D2;
constexpr int kSendButton = D1;
constexpr int kClearButton = D0;
constexpr uint32_t kDebounceMs = 25;
constexpr uint32_t kSendRearmMs = 250;
...
DebouncedButton talkButton{kTalkButton};
...
void setup() {
  ...
  pinMode(kSendButton, INPUT_PULLDOWN);
  pinMode(kClearButton, INPUT_PULLDOWN);
  initMicrophone();
  ...
  USB.productName("XIAO Voice Keyboard V3");
...
void loop() {
  ...
  const bool sendChanged = updateButton(sendButton, now);
  ...
  // 250 ms. K3 also locks K2 out so adjacent-key/cabling crosstalk cannot send.
  if (clearButton.pressed || clearChanged) {
    sendArmed = false;
  } else if (!sendButton.pressed && !sendButton.rawPressed &&
             now - sendButton.changedAt >= kSendRearmMs &&
             now - clearButton.changedAt >= kSendRearmMs) {
    sendArmed = true;
  }

  if (sendChanged && sendButton.pressed && sendArmed &&
      !talkButton.pressed && !clearButton.pressed) {
    sendArmed = false;
    keyboard.write(KEY_RETURN);
  }

  if (clearButton.pressed && !clearButton.longActionDone &&
      !talkButton.pressed && now - clearButton.pressedAt >= kClearHoldMs) {
    clearButton.longActionDone = true;
