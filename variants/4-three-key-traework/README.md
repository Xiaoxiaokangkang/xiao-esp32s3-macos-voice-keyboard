# V4：调用 TraeWork CN、语音/切换输入法、发送

V4 面向 TRAE SOLO CN 工作流。它沿用 V3 的 USB 音频底层和硬件接线，但重新定义了三个按键，并增加 TraeWork Bridge。

设备连接后显示为 `XIAO Voice Keyboard V4 TraeWork`，可与 V2、V3 明确区分。
...
1. 如果开发板尚未刷入这个版本，运行 `firmware/烧录V4.command`。
...
- 完整镜像：`firmware/XIAO-ESP32S3-Voice-Keyboard-V4-complete.bin` → `0x0`
- `bootloader.bin` → `0x0000`
- `partitions.bin` → `0x8000`
- `boot_app0.bin` → `0xE000`
- `firmware-app-V4.bin` → `0x10000`
