...
| 顺序 | 课程版本 | 硬件 | 按键功能 |
|---:|---|---|---|
| 1 | [V1：K1 语音输入](variants/1-three-key-k1-voice/README.md) | 3 个按键 | K1 语音输入，K2/K3 暂未启用 |
| 2 | [V2：语音、发送、取消](variants/2-three-key-voice-send-cancel/README.md) | 3 个按键 | K1 语音、K2 发送、K3 长按取消/清空 |
| 3 | [V3：TraeWork CN](variants/3-three-key-traework/README.md) | 3 个按键 | K1 调用 TraeWork、K2 语音/切换输入法、K3 发送 |
...
V1、V2 和 V3 均提供固件源码、固定版本的 PlatformIO 配置、Mac 端源码、App Info.plist 和构建脚本。用户既可以直接使用预编译文件，也可以从源码重新构建。完整步骤见 [构建说明](docs/BUILDING.md)。
...
- V1、V2 和 V3 使用相同的三键硬件与麦克风接线，但按键逻辑和 USB 产品名不同。
- USB 产品名分别显示为 `XIAO Voice Keyboard V1`、`XIAO Voice Keyboard V2` 和 `XIAO Voice Keyboard V3 TraeWork`。
