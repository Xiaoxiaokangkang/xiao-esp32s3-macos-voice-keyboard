...
constexpr uint8_t kSystemFunctionShiftReportId = 1;

// Test A: Generic Desktop / System Control / System Function Shift (0x97).
// The one data bit is a state: 1 while K1 is held and 0 after K1 is released.
// A distinct report ID keeps the framing explicit on the composite HID device.
constexpr uint8_t kSystemFunctionShiftReportDescriptor[] = {
    0x05, 0x01,  // Usage Page (Generic Desktop)
    0x09, 0x80,  // Usage (System Control)
    0xA1, 0x01,  // Collection (Application)
    0x85, kSystemFunctionShiftReportId,  //   Report ID (1)
...
class SystemFunctionShiftHID : public USBHIDDevice {
 public:
  SystemFunctionShiftHID() : hid_() {
    USBHID::addDevice(this, sizeof(kSystemFunctionShiftReportDescriptor));
  }
  ...
  bool setPressed(bool pressed) {
    ...
    return hid_.SendReport(kSystemFunctionShiftReportId, &report,
                           sizeof(report));
  }

  uint16_t _onGetDescriptor(uint8_t *buffer) override {
    memcpy(buffer, kSystemFunctionShiftReportDescriptor,
           sizeof(kSystemFunctionShiftReportDescriptor));
    return sizeof(kSystemFunctionShiftReportDescriptor);
  }
...
SystemFunctionShiftHID systemFunctionShift;
...
void setup() {
  ...
  // Use a different identity from the Known-Good V1 so macOS does not reuse
  // the cached F13 keyboard report descriptor while evaluating this test.
  USB.PID(0x005E);
  USB.firmwareVersion(0x0A01);
  USB.productName("XIAO System Function Shift Test A");
...
void loop() {
  ...
  if (reportPending && systemFunctionShift.ready()) {
    reportPending = !systemFunctionShift.setPressed(stablePressed);
  }
  delay(2);
}
